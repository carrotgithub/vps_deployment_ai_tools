...
| **三模式支持** | 支持域名模式、IP 模式和 HTTP 模式 |

### 访问模式说明

本项目所有服务脚本（V2Ray、CliproxyAPI、New-API、LiteLLM、CN2-Proxy）均支持三种访问模式：

| 模式 | SSL 证书 | 适用场景 | 注意事项 |
|------|----------|----------|----------|
| **域名模式** | Let's Encrypt（自动申请） | 生产环境（推荐） | 需要域名解析到服务器 |
| **IP 模式** | 自签名证书 | 测试环境/无域名场景 | 浏览器会提示不安全，客户端需信任证书 |
| **HTTP 模式** | 无 | 内网/开发环境 | 数据传输不加密，仅限受信任网络 |

运行安装脚本时，会提示选择访问模式：
```
>>> 请选择访问方式
  1) 使用域名（推荐）- 自动申请 Let's Encrypt 证书
  2) 使用 IP 地址   - 自签名证书，无需域名
  3) 仅使用 HTTP    - 无 SSL 证书，仅限内网/开发环境
```
...
- **域名（可选）**: 申请 SSL 证书需要已解析的域名；也支持 IP 模式（自签名证书）
